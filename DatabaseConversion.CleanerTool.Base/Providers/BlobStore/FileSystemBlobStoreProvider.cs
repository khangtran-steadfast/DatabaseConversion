using DatabaseConversion.CleanerTool.Base.DatabaseMapper.Exceptions;
using System;
...
namespace DatabaseConversion.CleanerTool.Base.DatabaseMapper.Providers.BlobStore
{
    ...
    public class FileSystemBlobStoreProvider : BlobStoreProvider
    {
        ...
        public FileSystemBlobStoreProvider()
        {
            ...
            {
                throw new ConversionException(ConversionExceptionCodes.SERVICE_ERROR_APP_SETTING, APP_SETTING_ROOT_DIR, _rootPath);
            }
        ...
        private void CreateFile(string category, string name, out string key, out FileInfo file)
        {
            ...
            { throw new ConversionException(ConversionExceptionCodes.BLOB_STORE_PROVIDER_ERROR_CATEGORY_SYNTAX, "", category); }
            if (!Regex.Match(name, "^[A-Za-z0-9\\._-]+$").Success)
            { throw new ConversionException(ConversionExceptionCodes.BLOB_STORE_PROVIDER_ERROR_NAME_SYNTAX, "", name); }
        ...
        private string UriToPath(string key, bool checkIfFileExists, bool create)
        {
            ...
            { throw new ConversionException(ConversionExceptionCodes.BLOB_STORE_PROVIDER_ERROR_INVALID_KEY, "null or empty", key); }

            string[] parts = key.Split(':');
            if (parts.Length != 4)
            { throw new ConversionException(ConversionExceptionCodes.BLOB_STORE_PROVIDER_ERROR_INVALID_KEY, "4 parts expected", key); }

            string category = parts[0];
            string ts = parts[1];
            if (ts.Length != 10)
            { throw new ConversionException(ConversionExceptionCodes.BLOB_STORE_PROVIDER_ERROR_INVALID_KEY, "timestamp format not 'yyyyMMddHH'", key); }
            ...
            if (!dir.Exists)
            {
                ...
                {
                    throw new ConversionException(ConversionExceptionCodes.BLOB_STORE_PROVIDER_ERROR_NOT_FOUND, key);
                }
            }
        ...
        public override string PutBlob(string category, string name, Stream contents)
        {
            ...
            if (file.Exists)
            { throw new ConversionException(ConversionExceptionCodes.BLOB_STORE_PROVIDER_ERROR_EXISTS, file.FullName); }
