...
UPDATE policies 
SET pol_insurer = wor_insurer
FROM dbo.policies INNER JOIN dbo.workbooks ON workbooks.wor_id = policies.pol_wor_id
WHERE pol_tran_id IS NULL

update	policies
set	pol_transaction_type = CONVERT(int, attv_value)
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_transaction_type'

update	policies
set	pol_date_from = CONVERT(smalldatetime, attv_value)
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_date_from'

update	policies
set	pol_date_to = CONVERT(smalldatetime, attv_value)
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_date_to'

update	policies
set	pol_date_effective = CONVERT(smalldatetime, attv_value)
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_date_effective'

update	policies
set	pol_sum_insured = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_sum_insured'

update	policies
set	pol_insured = attv_value
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_insured'

update	policies
set	pol_total_base_premium = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_total_base_premium'

update	policies
set	pol_total_levies = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_total_levies'

update	policies
set	pol_total_duties = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_total_duties'

update	policies
set	pol_total_fees = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_total_fees'

update	policies
set	pol_total_gross_premium = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_total_gross_premium'

update	policies
set	pol_total_commission = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_total_commission'

update	policies
set	pol_comm_net_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_comm_net_gst'
...
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_total_net_premium'

update	policies
set	pol_base_premium = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_base_premium'

update	policies
set	pol_uw_fee_net_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_uw_fee_net_gst'

update	policies
set	pol_uw_fee_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_uw_fee_gst'

update	policies
set	pol_fee_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_fee_gst'

update	policies
set	pol_fee_net_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_fee_net_gst'

update	policies
set	pol_insurer_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_insurer_gst'

update	policies
set	pol_policy_number = attv_value
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_policy_number'

update	policies
set	pol_hvi_value = CONVERT(bigint, attv_value)
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_hvi_value'

update	policies
set	pol_limited_exemption = CONVERT(int, attv_value)
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_limited_exemption'

update	policies
set	pol_exemption_type = CONVERT(int, attv_value)
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_exemption_type'

update	policies
set	pol_cover_note_number = attv_value
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_cover_note_number'

update	policies
set	pol_location = attv_value
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_location'

update	policies
set	pol_comm_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_comm_gst'
...
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_sub_agent_comm_payable_gst'

update	policies
set	pol_sub_agent_comm_pay_net_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_sub_agent_comm_pay_net_gst'

update	policies
set	pol_sub_agent_fee_payable_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_sub_agent_fee_payable_gst'

update	policies
set	pol_sub_agent_fee_pay_net_gst = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_sub_agent_fee_pay_net_gst'
...
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_sub_agent_comm_payable'

update	policies
set	pol_sub_agent_fee_payable = CONVERT(money, coalesce(attv_value,'0.00'))
from	policies
	join atura_tagvalue on attv_row_id = pol_wor_id
where	pol_tran_id IS NULL AND attv_tag = 'pol_sub_agent_fee_payable'
...
select	pol_id from policies
order by pol_id

open workbook_cursor

fetch next from workbook_cursor into @pol_id
while (@@FETCH_STATUS = 0)
begin
	DECLARE text_cursor CURSOR FOR
	SELECT	attv_index, attv_value
	FROM	atura_tagvalue
	WHERE	attv_row_id = @pol_id
